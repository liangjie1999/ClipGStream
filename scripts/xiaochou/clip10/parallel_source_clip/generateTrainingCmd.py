...
### You need set ###
cuda_idxs = [0, 1, 2, 3, 4, 5, 6, 7]
source_path = "/data2/dataset/xiaochou"

model_path = "./output/experiment/xiaochou/clip_size/10/"
ply_path = "plys/10/residualClipPly"       
config_path = "arguments/xiaochou/basketball.py"
downsample = "2.0"
project_total_frames = 1500
clip_size = 10
...
for i in range(start_gop, clip_nums):
    ...
    cmd = f"CUDA_VISIBLE_DEVICES={cuda_idx} python trainSourceClip.py --project_total_frames {project_total_frames} --clip_size {clip_size} --iterations {iterations} -s {source_path} -m {model_path} --frames_start_end {start_idx} {end_idx} --configs {config_path} --downsample {downsample} --ply_path {ply_path}"

    cmd_idx = (i - start_gop) % parallell_interval
    cmds[cmd_idx].append(cmd)


output_root = './training_command'
import os
