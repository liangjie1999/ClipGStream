...
for i in range(start_gop, clip_nums):
    ...
    cmd = f"CUDA_VISIBLE_DEVICES={cuda_idx} python trainSourceClip.py --project_total_frames {project_total_frames} --clip_size {clip_size} --iterations {iterations} -s {source_path} -m {model_path} --frames_start_end {start_idx} {end_idx} --configs arguments/tiny/basketball.py"
