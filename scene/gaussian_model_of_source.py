...
class SourceClipGaussianModel:
    referenceClipGaussianModel: ReferenceClipGaussianModel
    ...
    def __init__(self, referenceClipGaussianModel: ReferenceClipGaussianModel,
                 args, opt,
                 feat_dim: int=32, 
                 n_offsets: int=5, 
        ...
        self.referenceClipGaussianModel = referenceClipGaussianModel
        ...
        self.frame_0_mask = None
        self.iteration = iteration
        self.gop_slice = []

    def create_source_STF(self, training_args):
        ...
        if self.hash : # True
            ...
            self.dynamic_module = SpaceTimePlaneField ( self.args, self.get_xyz_bound()[0] , self.get_xyz_bound()[1],  self.feat_dim, clip_id=clip_id)
    ...
    # 获取属性 将获得reference clip和source clip合并的结果
    @property
    def get_scaling(self):
        return torch.cat((self.referenceClipGaussianModel.get_scaling.cuda(), 1.0*self.scaling_activation(self._scaling)), dim=0)
    
    @property
    def get_featurebank_mlp(self):
        return self.referenceClipGaussianModel.mlp_feature_bank
    
    @property
    def get_opacity_mlp(self):
        return self.referenceClipGaussianModel.mlp_opacity
    
    @property
    def get_cov_mlp(self):
        return self.referenceClipGaussianModel.mlp_cov


    @property
    def get_color_mlp(self):
        return self.referenceClipGaussianModel.mlp_color
    
    @property
    def get_offset_mlp(self):
        return self.referenceClipGaussianModel.mlp_offset
    
    @property
    def get_rotation(self):
        return torch.cat((self.referenceClipGaussianModel.get_rotation.cuda(),  self.rotation_activation(self._rotation)), dim=0)      
    
    @property
    def get_anchor(self):
        return torch.cat((self.referenceClipGaussianModel.get_anchor.cuda(),  self._anchor), dim=0)      
    
    @property
    def get_anchor_feat(self):
        return torch.cat((self.referenceClipGaussianModel.get_anchor_feat.cuda(), self._anchor_feat), dim=0)
    ...
    def get_opacity(self):
        return torch.cat((self.referenceClipGaussianModel.get_opacity.cuda(),  self.opacity_activation(self._opacity)), dim=0) 
    
    #  ******************* dynamic 
    @property
    def get_dy_opacity_mlp(self):
        return self.referenceClipGaussianModel.mlp_opacity_dy
    
    @property
    def get_dy_cov_mlp(self):
        return self.referenceClipGaussianModel.mlp_cov_dy

    @property
    def get_dy_color_mlp(self):
        return self.referenceClipGaussianModel.mlp_color_dy
    
    @property
    def get_dy_offset_mlp(self):
        return self.referenceClipGaussianModel.mlp_offset_dy
    ...
    def create_source_anchors_with_reference(self, pcd: BasicPointCloud, spatial_lr_scale: float = 1.0):
        self.spatial_lr_scale = spatial_lr_scale
        points = pcd.points[::]


        # A_n^r
        source_residual_anchors = torch.tensor(np.asarray(points)).float().cuda()
        # A_0
        reference_anchors = torch.tensor(self.referenceClipGaussianModel._anchor.detach())
        
        # A_n = [A_0; A_n^r] = [fused_point_cloud; reference_anchors]
        source_anchors = torch.cat((reference_anchors.cuda(), source_residual_anchors), dim=0)
        source_anchors_dist2 = torch.clamp_min(distCUDA2(source_anchors).float().cuda(), 0.0000001)
        
        dist2 = source_anchors_dist2[:source_residual_anchors.shape[0]]
        mask = dist2 < 10
        scales = torch.log(torch.sqrt(dist2[mask]))[...,None].repeat(1, 6) 

        offsets = torch.zeros((source_residual_anchors.shape[0], self.n_offsets, 3)).float().cuda()

        if self.opt.open_feat_cat:
            anchors_feat = torch.zeros((source_residual_anchors.shape[0], self.feat_dim // 2)).float().cuda()  # [N,32]
        else:
            anchors_feat = torch.zeros((source_residual_anchors.shape[0], self.feat_dim)).float().cuda()

        rots = torch.zeros((source_residual_anchors.shape[0], 4), device="cuda")
        rots[:, 0] = 1

        opacities = inverse_sigmoid(0.1 * torch.ones((source_residual_anchors.shape[0], 1), dtype=torch.float, device="cuda")) # [N,1]

        self._anchor = nn.Parameter(source_residual_anchors[mask], requires_grad=True)
        self._offset = nn.Parameter(offsets[mask], requires_grad=True)
        self._anchor_feat = nn.Parameter(anchors_feat[mask], requires_grad=True)
        self._scaling = nn.Parameter(scales, requires_grad=True)
        self._rotation = nn.Parameter(rots[mask], requires_grad=False)
        self._opacity = nn.Parameter(opacities[mask], requires_grad=False)

        self.maxz, self.minz = torch.amax(self.get_anchor[:,2]), torch.amin(self.get_anchor[:,2]) 
        self.maxy, self.miny = torch.amax(self.get_anchor[:,1]), torch.amin(self.get_anchor[:,1]) 
        self.maxx, self.minx = torch.amax(self.get_anchor[:,0]), torch.amin(self.get_anchor[:,0]) 
        self.maxz = min((self.maxz, 200.0)) # some outliers in the n4d datasets.. 

    def create_reference_anchors_mask(self):
        total_anchors_len = len(self.get_anchor)
        
        reference_anchors_mask = torch.zeros((total_anchors_len, ), dtype=torch.bool)

        reference_anchors_mask[0:len(self.referenceClipGaussianModel.get_anchor)] = True
        
        self.reference_anchors_mask = reference_anchors_mask
        return self.reference_anchors_mask


    def get_xyz_bound(self, percentile = 100.0):
        with torch.no_grad():
            half_percentile = (100 - percentile) / 200
            self._xyz_bound_min = torch.quantile(self.get_anchor , half_percentile,dim=0)  # 86.6 % ?
            self._xyz_bound_max = torch.quantile(self.get_anchor , 1 - half_percentile,dim=0)
    ...
    def training_setup(self, training_args, stage):
        self.create_source_STF(training_args)                    # create source clip's Spatio Temporal Field (STF)
        ...
        # optimizer 如何删掉一部分属性呢 这里只能存入一个非叶子 那还是得copy
    ...
    def save_ply(self, path):
        self.save_ply_source_clip(path)


    def save_ply_source_clip(self, path):
        clip_id = self.clip_id
        ...
        dynamic_mask = torch.ones((self._anchor.shape[0], 1)).cpu().numpy()
    ...
    def load_dynamic_module(self):
        clip_id = self.clip_id
        if self.hash:
            dynamic_path = os.path.join(self.model_path, 'history', f'FDHash_{clip_id}.pth')
            ...
            weight_dict = torch.load(dynamic_path, map_location="cpu")
            dynamic_module.load_state_dict(weight_dict)

            self.dynamic_module = dynamic_module

        else:
            dynamic_path = os.path.join(self.model_path, 'history', f'Planes_{clip_id}.pth')
...
#$   * ************************************************
    ...
    def load_model(self, path, iteration=None):
        import math
        total_frames = self.frames_start_end[1] - self.frames_start_end[0]
        clip_size = self.opt.clip_size
        gop_num = math.ceil(total_frames / clip_size)

        if iteration is not None:
            per_clip_iter = int(os.environ.get('per_clip_iter', 10000))

            gop_num_limit = iteration // per_clip_iter

            gop_num = min(gop_num, gop_num_limit)

        history_path = os.path.join(self.model_path, 'history')

        if self.hash:
            if bool(os.environ.get('open_history_dynamic', True)):
            ...
            self.n_levels ,  self.n_features_per_level,  self.base_resolution , self.n_neurons , self.feat_dim, open_feat_cat=self.opt.open_feat_cat )
                    ...
                    weight_dict = torch.load(os.path.join(history_path, f"FDHash_{i}.pth"),map_location="cpu")
                    dynamic_module.load_state_dict(weight_dict)

                    self.history_dynamic_modules.append(dynamic_module)
            ...
            if bool(os.environ.get('open_history_dynamic', True)):
                for i in range(gop_num):
                    # TODO:BUG 这里要写成平面
                    dynamic_module = SpaceTimePlaneField( self.args, self.get_xyz_bound()[0],self.get_xyz_bound()[1], self.feat_dim )
                    weight_dict = torch.load(os.path.join(history_path, f"Planes_{i}.pth"),map_location="cpu")
                    dynamic_module.load_state_dict(weight_dict)
