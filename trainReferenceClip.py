...
from scene import ReferenceClipScene, ReferenceClipGaussianModel
...
def training(dataset, hyper, opt, pipe, frames_start_end, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname,args, load_iteration=None):
    ...
    gaussians = ReferenceClipGaussianModel( hyper, opt, dataset.feat_dim, 10, dataset.voxel_size, dataset.update_depth, dataset.update_init_factor, dataset.update_hierachy_factor, dataset.use_feat_bank, 
                              dataset.appearance_dim, dataset.ratio, dataset.add_opacity_dist, dataset.add_cov_dist, dataset.add_color_dist, frames_start_end=frames_start_end, model_path=dataset.model_path)
    
    dataset.model_path = args.model_path
    timer = Timer()

    scene = ReferenceClipScene(dataset, gaussians, frames_start_end, load_iteration=load_iteration)
    timer.start()
...
def scene_reconstruction(dataset, hyper, opt,  pipe, testing_iterations, saving_iterations, 
    ...
    if load_iteration and checkpoint:
        assert False, "Only one of load_iteration and checkpoint can be passed"
    ...
    if load_iteration:
        gaussians.load_optimizer(os.path.join(args.model_path,
                                                           "point_cloud",
    ...
    if clip_nums != 1:
        assert False, "training reference clip only support one clip."

    if True:
        viewpoint_stack = scene.getTrainCamerasByCipId(cur_clip)    #
        ...
        for iteration in range(0, per_clip_iter):
            iter_start.record()
            if  opt.dataloader and not load_in_memory: # here
                try:
            ...
            voxel_visible_mask = prefilter_voxel(viewpoint_cam, gaussians, pipe, background, clip_id=cur_clip, primitive_type=primitive_type)  # 判断是否可见 
            retain_grad = (iteration < opt.update_until and iteration >= 0)

            render_pkg = render( viewpoint_cam, gaussians, pipe, background, iteration=iteration, stage=stage, visible_mask=voxel_visible_mask, retain_grad=retain_grad, clip_id=0, primitive_type=opt.primitive_type)
            ...
            if os.environ.get('debug', True):
                if iteration % 1000 == 1:
                    ...
                    # cv2 bgr; torch bgr
                    cv2.imwrite(f'{args.model_path}/debug/{cur_clip}/{cur_clip * per_clip_iter + iteration}.jpg', display_image)
            ...
            # If the primitive is 2dgs, geometric loss needs to be increased by adding lambda normal and lambda dist
            ...
            for param in gaussians.dynamic_module.parameters():  # Prevent gradient explosion or numerical anomalies
            ...
            with torch.no_grad():
                ...
                if stage == "fine" :
                    training_report(args, gaussians, tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), \
                                    testing_iterations, scene, render, [ pipe , background  ], stage, scene.dataset_type, logger, primitive_type=primitive_type)
                ...
                elif iteration == opt.update_until:  # 15000 clear cache
...
def training_report(args , gaussians, tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, \
                    scene, renderFunc, renderArgs, stage, dataset_type, logger, primitive_type='3dgs'):
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:    # 测试视角居然有100个？这也太奇怪了
                l1_test = 0.0
                psnr_test = 0.0
                for idx, viewpoint in enumerate(config['cameras']):
                    voxel_visible_mask = prefilter_voxel(viewpoint, gaussians, renderArgs[0], renderArgs[1], primitive_type=primitive_type)  # 判断是否可见 
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians,stage=stage, iteration = iteration, visible_mask = voxel_visible_mask, 
                                                   retain_grad = True, primitive_type=primitive_type, *renderArgs)["render"], 0.0, 1.0)
                    
                    
                    # image = torch.clamp(renderFunc( viewpoint, gaussians, renderArgs[0], renderArgs[1], primitive_type=primitive_type, iteration=iteration,
                    #                         stage=stage, visible_mask=voxel_visible_mask, clip_id=0)["render"], 0.0, 1.0)
                    # render( viewpoint_cam, gaussians, pipe, background, iteration=iteration, stage=stage, visible_mask=voxel_visible_mask, retain_grad=retain_grad, clip_id=0, primitive_type=opt.primitive_type)
                    gt_image = torch.clamp(viewpoint.original_image.to("cuda"), 0.0, 1.0)

                    if config['name'] == 'test' and iteration > 0:
                        os.makedirs(f'{gaussians.model_path}/renders_middle/0/{iteration}/', exist_ok=True)
                        torchvision.utils.save_image(
                            image,
                            f'{gaussians.model_path}/renders_middle/0/{iteration}/{idx:0>5}.jpg'
                        )
